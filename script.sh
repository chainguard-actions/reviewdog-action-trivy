...
echo "::group::🐶 Installing reviewdog (${REVIEWDOG_VERSION}) ... https://github.com/reviewdog/reviewdog"
  ...
  curl -sfL https://raw.githubusercontent.com/reviewdog/reviewdog/9b54cccfb4bf2509aef8a3e26899412348b62ce9/install.sh -o "${INSTALL_SCRIPT}"
  GITHUB_TOKEN="${INPUT_GITHUB_TOKEN}" sh "${INSTALL_SCRIPT}" -b "${REVIEWDOG_PATH}" "${REVIEWDOG_VERSION}" 2>&1
  rm -f "${INSTALL_SCRIPT}"
echo '::endgroup::'
...
echo '::group:: Running trivy with reviewdog 🐶 ...'
  ...
  if [ -n "${INPUT_TRIVY_FLAGS:-}" ]; then
    while IFS= read -r -d '' t; do trivy_flags+=("$t"); done \
      < <(printf '%s' "${INPUT_TRIVY_FLAGS}" | xargs printf '%s\0')
  fi

  trivy_command=()
  if [ -n "${INPUT_TRIVY_COMMAND}" ]; then
    while IFS= read -r -d '' t; do trivy_command+=("$t"); done \
      < <(printf '%s' "${INPUT_TRIVY_COMMAND}" | xargs printf '%s\0')
  fi

  trivy_target=()
  if [ -n "${INPUT_TRIVY_TARGET}" ]; then
    while IFS= read -r -d '' t; do trivy_target+=("$t"); done \
      < <(printf '%s' "${INPUT_TRIVY_TARGET}" | xargs printf '%s\0')
  fi
  ...
  "${TRIVY_PATH}/trivy" --format sarif "${trivy_flags[@]}" --exit-code 1 "${trivy_command[@]}" "${trivy_target[@]}" 2> /dev/null \
