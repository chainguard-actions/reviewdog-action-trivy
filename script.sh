...
echo "::group::🐶 Installing reviewdog (${REVIEWDOG_VERSION}) ... https://github.com/reviewdog/reviewdog"
  ...
  curl -sfL --output "${REVIEWDOG_INSTALL_SCRIPT}" https://raw.githubusercontent.com/reviewdog/reviewdog/fd59714416d6d9a1c0692d872e38e7f8448df4fc/install.sh
...
echo '::group:: Running trivy with reviewdog 🐶 ...'
  ...
  # Build trivy flags array safely to avoid word-splitting / shell injection
  trivy_flags_array=()
  if [[ -n "${INPUT_TRIVY_FLAGS:-}" ]]; then
    IFS=' ' read -r -a trivy_flags_array <<< "${INPUT_TRIVY_FLAGS}"
  fi

  # Build reviewdog flags array safely
  reviewdog_flags_array=()
  if [[ -n "${INPUT_FLAGS:-}" ]]; then
    IFS=' ' read -r -a reviewdog_flags_array <<< "${INPUT_FLAGS}"
  fi
